...
>
> **상태: v9 (2026-08-09 갱신, 총괄). 제출 확정 — 남은 빈칸 없음.**
>
> **갱신분(v9) — 소환 행동 DSL 예시 삭제 (총괄 결정):**
> - "분신을 만들어서 지그재그로 돌진시켜라" 예시를 개요·표현의 폭·설계 주장에서 **전부 뺐다.**
> - **기능이 죽어서가 아니다.** 삭제 직전 실판정으로 확인했다 —
>   `HTTP 200 · 1,326ms · behavior.steps = ["zigzag","dash"]`. v5에서 "#319가 없앴다"고
>   적었던 것은 오진이었고(뺀 것은 **플레이어 이동**), 그 오진은 v7에서 이미 정정했다.
> - 뺀 이유는 **소개에서 중요한 자리가 아니라서**다. "설계하는 AI"를 둘(시간축·소환수 움직임)로
>   벌리면 각각이 얕아진다. 하나만 남기고 그 하나를 깊게 보이는 편이 낫다.
> - 대신 **시간축 예시를 개요의 대표 예시로 올렸다.** 이것도 실판정으로 확인 —
>   `"불덩이를 세 번 연달아, 마지막은 크게"` → sequences 4 · 작은 `bolt` ×3 + 큰 `nova` 피날레.
> - 소환 자체("내 분신을 만들어"·"화염 포탑"·"벌레 군체")는 표현의 폭에 그대로 남는다.
>
> **갱신분(v8):**
> - **§4 영상 링크 기입** — https://www.youtube.com/watch?v=jODg8jEvJs4 (R1 촬영·편집).
>   oEmbed로 공개 상태 확인(`incant_demo` / 이도원). 마지막 빈칸이 채워졌다
> - 스크린샷은 6장 → **4장**(#393). 정지 화면이 담을 수 있는 것만 남겼다 —
>   시간축 시퀀스와 자기 강화 전후 차이는 **영상**이 맡는다
> - 촬영 조건에서 프로덕션/개발 서버 구분 제거(#394). 같은 번들·같은 판정 경로라
>   결과가 동일한데 구분해 적으면 없는 차이를 만든다
>
> **갱신분(v7):**
...
>
> 변환 전 체크: ☑ 스크린샷 4장 삽입 ☑ 판정 p50·폴백률 기입(8/6 실측)
> ☑ **영상 링크 기입(§4)** — 8/9 완료
> ☑ 분량 — **8쪽**(본문 + 스크린샷). 종전 자체 기준 "≤6쪽"을 넘겼으나, 대회 요건에
> 쪽수 제한은 없고(제출 계획서 §1 확인) 스크린샷은 심사위원이 문서에서 가장 먼저 보는
> 것이라 축소하지 않았다.
<!-- /pdf:skip -->
...
> "얼음 감옥에 가둬라" → 빙결 속박 / "불덩이를 세 번 연달아, 마지막은 크게" → 작은 화염 3발 뒤 큰 폭발
...
> **설계하는 AI(위 굵은 행)가 이 게임의 핵심 주장이다.** LLM이 메뉴에서 하나를 고르는 게
> 아니라, 문장을 읽고 **시간축 위에 부품을 배치해** 돌려준다. 여러 단계를 순서와 간격까지
> 정하고, 명시적인 연사는 `wait` 간격으로, 동시에 일어나는 사건은 병렬 form으로 갈린다.
> *"불덩이를 세 번 연달아, 마지막은 크게"* → 작은 `bolt` 3단계 + 큰 `nova` 피날레.
>
...
같은 마법도 각성 전후가 다르다 — 예컨대 각성 친화로 세운 장벽은
보스의 돌진을 **버티고 서 있고**, 그 아래면 한 번 막고 부서진다.
...
| **플레이 영상** | https://www.youtube.com/watch?v=jODg8jEvJs4 |
