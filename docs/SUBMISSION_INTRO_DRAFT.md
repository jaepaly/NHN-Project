...
> **상태: 초안 v5 (2026-08-06 갱신, 총괄).** 문서·빌드·QA를 먼저 확정하고 영상은 마지막에 촬영한다(영상 촬영 담당: R1).
>
...
>
> 변환 전 체크: □ 최신 프로덕션 빌드 확인 □ 스크린샷 7장 □ 데모 URL 접속 확인
> □ 시퀀스·필살영창 재현 확인 □ **판정 p50·폴백률 재측정 기입** □ **영상 링크 기입(§4)**
> □ 페이지 수 ≤ 6
...
> "얼음 감옥에 가둬라" → 빙결 속박 / "불덩이를 세 번 연달아, 마지막은 크게" → LLM이 짠 박자대로 세 번 나간다
...
> **시간축 설계(위 4행)가 이 게임의 핵심 주장이다.** LLM이 메뉴에서 하나를 고르는 게
> 아니라, 문장을 읽고 **여러 단계를 순서와 간격까지 정해** 돌려준다. 명시적인 연사는
> `wait` 간격으로, 동시에 일어나는 사건은 병렬 form으로 갈린다.
>
> 안전은 로컬이 쥔다 — 실행 가능한 form이 없는 계획은 거부하고, 위력·마나·시간 제한은
> 로컬 검증기가 소유하며, 계약 위반 출력은 재판정한다. *창의는 LLM이, 규칙은 게임이.*
>
> (실측: 신규 묘사 30종 30/30 계약 준수 · 평균 1,443ms · p90 1,576ms)
...
- **목표**: 분기형 지도를 따라 8개의 방을 돌파해 마지막 방의 **기억의 주인**(최종
  보스)을 처치한다. 중간의 **수문장**(스테이지 보스)을 넘으면 런의 절반이다.
...
| **플레이 영상** | ⟨8/9 촬영 후 YouTube 링크 기입 — 확정 전 제출 금지⟩ |
...
- [ ] ② 창의 주문 발동 — 주문명 각인 + 이펙트
- [ ] ③ 자기 강화 — 무적/가속 상태창 표시
- [ ] ④ **`form/wait` 시퀀스 — 시간축 설계** *(핵심 컷: 한 문장이 박자를 갖고 여러 번 나간다)*
- [ ] ⑤ 기억 보스 — 저항 링 + 대사
...
- [ ] 조작표를 실제 빌드와 대조한다: `WASD` 이동, `Enter` 일반 영창, `Shift+Enter` 필살 진입, 보상 `A/D` · 경로 `W/S`.
