...
> **상태: 초안 v4 (2026-08-05 갱신, 총괄).** 문서·빌드·QA를 먼저 확정하고 영상은 마지막에 촬영한다(영상 촬영 담당: R1).
> 갱신분(v4): 조작표에 Shift+Enter 필살·ESC 일시정지 반영 · 판정 실측 수치를 리전 이전(#311) 후 재측정 항목으로 전환.
> 변환 전 체크: □ 최신 프로덕션 빌드 확인 □ 스크린샷 7장 □ 데모 URL 접속 확인 □ 조작표 최신화 □ 시퀀스·필살영창 재현 확인 □ **판정 p50·폴백률 재측정 기입** □ 페이지 수 ≤ 6
...
- 판정 실패·타임아웃 시 로컬 Mock 판정으로 **자동 폴백 — 게임은 절대 멈추지 않는다**.
  (실측 수치는 판정 프록시 리전 이전(#311/#350) 조치 후 **최종 확정 직전에 재측정해 기입** —
  ⟨판정 p50 __초 · 폴백 __/N회⟩)
...
- [ ] 조작표를 실제 빌드와 대조한다: `WASD` 이동, `Enter` 일반 영창, `Shift` 필살 영창 진입, 마우스 조준.
- [ ] 일반 영창의 단일 주문과 `form/wait` 시퀀스, 준비된 필살 게이지의 `ultimate plan` 실행을 각각 한 번 확인한다.
