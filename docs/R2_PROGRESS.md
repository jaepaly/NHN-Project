...
- [~] 최신 `main`과의 PR 병합 가능 상태는 후속 커밋·CI 완료 뒤 다시 확인한다.

**R2 작업**
- [x] ESC 일시정지 메뉴: 전체 경로 지도 아래와 필살영창 게이지 위의 여백이 균형을 이루도록 중앙 메뉴만 120px 아래로 재배치했다. 논리 좌표 기준 지도 아래 134px, 필살영창 게이지 위 150px 여백으로 맞췄으며 ESC·HUD·압축 HUD 회귀, production build, diff check를 통과했다. PR #347은 최신 main과 충돌 상태라 rebase·CI 확인 뒤 병합 가능 상태를 갱신한다.
...
- [~] 실플레이 일반방에서 빈 정화 안내 패널을 발견했다. 실제 정화 문구가 있을 때만 공간을 차지하도록 보정한 뒤 일반방·위험지대·보스방·ESC 검사 화면의 겹침과 가독성을 재확인하고 PR #347 CI·병합 가능 상태를 다시 확인한다.
