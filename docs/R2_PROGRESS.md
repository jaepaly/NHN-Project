...
| ③ | 보스 대사 생성 (프록시 `/boss-line` + **폴백 템플릿 필수**) | 🔧 (a)클라이언트·템플릿 폴백 ✅ / (b)프록시 엔드포인트 **← 다음** |
...
**▶ 다음 할 일**: **③(b) 프록시 `/boss-line` 엔드포인트 추가 + 배포**. ③(a) 클라이언트(getBossLine)·템플릿 폴백은 완료. worker.js에 경로 라우팅 추가 → 라이브 대사 생성.
...
- **(b) 프록시 ⬜**: `worker.js`에 `/boss-line` 경로 라우팅 추가 — 런 요약 → 위협 대사 프롬프트 → Gemini. 배포 후 라이브 연결.
