...
- [ ] 보물방 — **무전투 즉시 3택** (기존 `applyReward`/`RewardOption` 재포장)
- [ ] 제단방 — **HP 지불 → 상급 3택** (대가/보상 밸런스는 총괄·이도원 협의)
- [ ] R1 `MapGraph` 노드 타입에 편입 배선 (계약 PR 나온 뒤)

**③ 본작업 B — 바닥형 지형** (해저드 존 기전 재사용)
- [ ] 용암 — 해저드 리스킨(주황 + 틱 피해)
- [ ] 독지대 — 피해 낮게 + 이탈 후 도트 잔류
- [ ] 배치 좌표는 **R1 프리셋 데이터에서** (데이터 구동, R2는 기전만) · 함정(방)은 R1 몫
