...
## ☐ 지금 작업 흐름 — 필살기 만충 시 무마나 영창 회귀 수정 (2026-07-31)

> **목표**: 융합 게이지가 100%이고 판정된 영창이 필살기 조건(이중 원소)을 충족하면, 보유 마나가 0이어도 시전되어야 한다. 프롬프트·판정 스키마는 건드리지 않고 실제 게임의 all-plan 시퀀스 실행 경로만 바로잡는다.

**의존성·현재 상태**
- [x] 최신 `origin/main`과 동기화된 상태에서 `codex/fix-fusion-zero-mana-cast` 브랜치 생성.
- [x] 실플레이 로그에서 `cast` 뒤 `sequence_exec`가 없는 마나 거부 사례를 확인하고, all-plan 판정이 `runSequenceCast()`에서 필살기 확인보다 먼저 마나를 검사하는 원인을 격리.

**R2 작업**
- [x] 마나 0·융합 게이지 100%·이중 원소 시퀀스 영창 회귀 테스트 추가. 수정 전 `시퀀스 방출·마나 지점을 못 찾음` 실패로 재현.
- [x] 시퀀스 실행 경로에서도 필살기 방출을 마나 검사보다 먼저 판정하고 실제 소모 마나를 0으로 처리.
- [x] 시퀀스의 이동·다단계 의미는 유지하면서 대표 공격을 기존 필살기 위력·상태이상·연출로 격상.
- [x] `test:fusion` 10군·`test:sequence`·`test:emptyroomfusionvfx`·타입 검사·프로덕션 빌드 통과.
- [x] AI 사용 로그 갱신.
- [x] 커밋 `9a1046e` 푸시 및 PR #315 검토 요청.
- [~] 사용자 실플레이에서 마나 0·게이지 100%·이중 원소 영창 최종 확인 및 총괄 리뷰·머지 대기.
