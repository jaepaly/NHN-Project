...
**R2 작업**
- [~] 1단계: 우측 상단 `CURRENT ROOM` 레이더를 추가해 플레이어=파랑·적=빨강으로 매 프레임 갱신하고, 전체 경로 지도는 ESC/방 선택 화면에 보존했다. 좌표 투영·경계 고정 7군과 기존 미니맵·일시정지·UI 팔레트 회귀, 타입 검사·프로덕션 빌드는 통과했으며 실화면 겹침 확인만 남았다.
- [~] 2단계: 기존 `runElapsedMs` 기준을 그대로 쓰는 `00:00.0` 텍스트를 상단 중앙에 단독 배치하고 우측 상태 패널의 `RUN` 중복 행을 제거했다. 시간 포맷·일시정지 제외·단일 표시 회귀 6군과 타입 검사를 통과했으며 실화면 확인만 남았다.
- [~] 3단계: 보스전에서만 보스를 따라다니는 체력·페이즈·저항·관통·기억 패턴 패널을 체력바 반대편 아래에 표시하고, 일반방·보스 사망 시 숨겼다. 전용 9군·기존 보스 저항 7군·보스 6군·보스 아르카나 6군·production build·diff check를 통과했으며 실화면 겹침 확인만 남았다.
- [~] 4단계: HP·마나·실드를 210×65px로 축약해 우측 하단 빌드 칩 왼쪽 12px 간격에 옮기고 현재값/최대값, 열파 표시, 쿨다운 띠와 HP 30% 이하 `!` 적색 경고를 보존했다. 실플레이 피드백에 따라 30% 이하 진입·35% 이상 해제되는 단일 저체력 적색 맥동을 추가하고, 어두운 배경 가독성을 위해 맥동 알파만 30% 높였다. 전용 9군·함정·열파·컴팩트 HUD·UI 팔레트 회귀와 production build 통과, 최종 실화면 확인 대기.
- [~] 5단계: 8원소 4×2 정보량과 각성·합주 표시는 유지하면서 친화 패널 폭을 300→244px로 줄이고 행 간격·게이지·여백을 압축했다. 작은 화면 경계 고정까지 자동 검증 통과, 실화면 확인 대기.
- [~] 전용 보스 9군·컴팩트 HUD 10군과 레이더·타이머·친화·UI 팔레트·빌드 칩·설정·일시정지 회귀, 타입 검사·프로덕션 빌드·diff check를 통과했다. 일반·함정·엘리트·보스방 종합 실플레이만 남았다.
- [ ] Issue #345에 구현·검증 결과를 남기고 PR로 총괄 검토를 요청한다.
