...
- [ ] **🎯 제출물 ④ — AI 활용 기술 문서** (R2 단독 오너, 마감 8/9) — 목차·1부(게임 속 AI)/2부(AI로 만든 게임) 구성부터
- [ ] **§6 API 재점검** — 마나 환류(#121)로 시전율↑ 반영 (§6 본문 갱신 완료 → **커밋 필요**)
- [ ] **실뎀 로깅 검증** — 다음 플레이테스트에서 "같은 속성 뎀감 과한가"(보스내성×0.3 + #77 격상 + 반복 중첩) **숫자로 확정**. ProtoScene에 `type:"dmg"` breakdown 로깅 추가됨(dev, 미커밋)
- [ ] 자투리: **#67 §5** 재확인(마나 경제 진화 반영) · **#112**가 #77 계약(gimmicksUnlocked·bossDualResistance) 소비 무결성 확인
- [ ] 미커밋 정리: `R2_PROGRESS` 현행화 + `SUBMISSION_PLAN §6` 커밋
