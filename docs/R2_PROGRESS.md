...
## ▶ 다음 할 일 (2026-07-23 · 제출 국면)

> 판정 시스템 코어는 완결. R2 무게중심이 **"구축 → 제출물·심사 대비"** 로 이동.

- [x] **🎯 제출물 ④ — AI 활용 기술 문서** — **초안 1~3부 완료·팀 리뷰 요청** ([PR #130](https://github.com/jaepaly/NHN-Project/pull/130)). 1부 게임 속 AI(프롬프트 전문 v2.3·불신 설계·behavior DSL), 2부 AI로 만든 게임(도구·사례4·통계), 3부 에셋·OSS 출처 — 전부 실제 코드·로그·라이선스 인용. 심사 직전 **마크다운→PDF 변환**만 남음
- [x] **§6 API 재점검** — 마나 환류(#121)로 시전율↑ 반영 (이 PR에 §6 본문 갱신 포함)
- [ ] **🎯 실뎀 로깅 검증 (다음 우선순위)** — 다음 플레이테스트에서 "같은 속성 뎀감 과한가"(보스내성×0.3 + #77 격상 + 반복 중첩) **숫자로 확정**. 실뎀 breakdown 로깅은 stale ProtoScene(774줄 변경)이라 폐기·diff 백업(`scratchpad/protoscene-dmg-logging.diff`) → **새 ProtoScene에 재적용 후** 한 판
- [ ] 자투리: **#67 §5** 재확인(마나 경제 진화 반영) · **#112**가 #77 계약(gimmicksUnlocked·bossDualResistance) 소비 무결성 확인
- [ ] 열린 docs PR 정리: #126 HANDOFF · #127 R2_PROGRESS+§6 · #130 ④ — 리뷰·머지 흐름 관리
