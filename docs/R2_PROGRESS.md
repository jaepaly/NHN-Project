...
- [ ] **실플레이 시퀀스 튜닝 (트리거는 실제 플레이 피드백)** — 게이트·held-out은 통과했으나, 고정셋 100%는 **의도적으로 안 함**(과적합). 실제 플레이에서 "이런 문장이 안 쪼개진다" 싶으면 `logs/play.jsonl`에 남은 실제 케이스로 few-shot 보강(held-out 재검증). **지연 여유 빠듯(max 2.45s)이라 예시 추가는 신중히.**
