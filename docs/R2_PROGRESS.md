...
## 완료 — #360 기억 보스 제출 스크린샷 추가 (PR #391, 2026-08-09)

> PR #391에 확보한 production 원본 5장에 이어, 정상 플레이로 기억의 주인까지 진행해 저항 정보와 도발 대사가 함께 읽히는 보스 컷을 추가한다. DEV·fixture·화면 합성은 사용하지 않는다.

production 실제 플레이로 기억의 주인 PHASE 1에 진입해 상단 화염 저항 `-15%`, 중앙 저항 안내, 지난 런 11회 사망을 인용한 도발 대사가 동시에 읽히는 장면을 확보했다. Xbox Game Bar 원본에서 게임 화면만 1920×1080으로 추출했으며 DEV·fixture·AI 생성·화면 합성은 사용하지 않았다. `docs/submission-screenshots/05-memory-boss.png`와 촬영 기록을 PR #391에 추가했다. 필수 ①~⑥은 모두 확보했고 ⑦ 친화도 비교는 이슈 원문대로 선택 컷으로 남겼다.

---

## 완료 — 제출물 ④ AI 기술 문서 정정·제출 스크린샷 준비 (#362, PR #391, 2026-08-09)

> 총괄 요청: 최신 main 기준으로 제출물 ④의 오래된 AI 판정 설명을 실제 배포 계약과 일치시키고 PDF를 다시 만든다. 제출물 ③에 필요한 실제 플레이 스크린샷 7컷의 요구사항을 확인해 production 촬영 환경과 원본 컷을 준비한다. 기존 보스 대사 미커밋 작업은 별도 작업 폴더에 보존하며 이번 브랜치에 섞지 않는다.

최신 `origin/main`(`ed718e4`)에서 전용 브랜치를 분리해 #362의 오래된 AI 판정 설명을 현행 `meaning-v2.31-ultimate-resonance-echo` 계약으로 정정했다. player `move` 제거와 summon `behavior` 유지의 경계를 명시하고, 타임아웃 재시도·RunRecoveryJudge·모델 핀·15 RPM·도쿄 placement·AI 로그 386건을 반영했다. PDF는 8페이지로 재생성해 전 페이지를 렌더링 검수했다.

#360용 production 1920×1080 실제 플레이 원본은 영창 모드·창의 주문·자기 강화·form/wait 시퀀스·보상 3택 5장을 확보했다. 정상 런 완주가 필요한 기억 보스 컷과 선택 친화도 비교 컷은 재현 절차를 기록했다. production build와 diff check를 통과했고 PR #391 및 #360·#362 코멘트로 전달했다.
...
## ☑ 지금 작업 흐름 — 보스 대사·알림 채널 분리 (PR #397, 2026-08-09)

> 보스 등장 대사가 일반 시스템 알림처럼 읽히는 문제를 해결한다. 이전에 구현과 자동 검증까지 마쳤지만 커밋·업로드 전에 중단돼 로컬에 남아 있던 변경을 최신 `main`에 통합해 별도 PR로 전달한다. 전투 수치·보스 패턴·주문 판정 프롬프트는 변경하지 않는다.

**의존성·범위**

- [x] 최신 `main` (`f477265`)을 전용 브랜치에 통합하면서 개인 자동 각인 에셋 실험 파일은 PR 범위에서 제외했다.
- [x] 보스 등장 대사·보스 행동 알림·일반 시스템 알림의 현재 표시 경로와 문구를 분류했다: 최종 보스의 AI 대사는 일반 시스템 공지에 따옴표만 붙여 출력되고, 등장 배너·내성·하수인·역영창 경고가 서로 다른 성격인데 중앙 공지 채널에 섞여 있다.
- [x] 기억의 주인 전용 문체를 확정·구현했다: 한 문장, 직접 호명(`너`/`네`), 전적 하나의 관찰→위협, 시스템 용어·수치·해설 금지. Gemini 프롬프트와 클라이언트 검증·결정론 템플릿에 같은 규칙을 둔다.
- [x] 일반 중앙 공지 대신 화자명·인장·인용문을 가진 전용 대사판을 구현했다. 수문장 등장 배너와 내성·소환·역영창 전투 정보는 기존 채널을 유지한다.

**후속 구현·검증**

- [x] 보스 대사 회귀, 전체 회귀 119종, TypeScript·production build·Worker 구문·`diff --check`를 최신 `main`에서 통과했다.
- [~] 전용 대사판의 최종 위치·가독성은 PR 리뷰 또는 로컬 실플레이에서 확인한다.
- [x] PR #397을 만들어, 이전에 구현했지만 업로드하지 못한 작업임을 밝히고 팀 검토를 요청했다.
