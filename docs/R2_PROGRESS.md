...
## ☐ 지금 작업 흐름 — 일반 생성 맵 비대칭 대각선 경로 (2026-07-31)

> **목표**: 일반 런의 랜덤 생성 맵에서 중간 lane 전환 선택을 늘린다. 고정 시연 프리셋은 보존하고, 인접한 연속 분기 layer에만 X자로 교차하지 않는 대각선을 맵 규모에 따라 기본 2~4개 배치한다.

**범위·기준**
- [x] 일반 런 `generateRunMap()`에만 적용하고 `MAP_GRAPH_PRESET_01` 시연 맵은 변경하지 않음.
- [x] 대각선 배치 용량 2/3/4+에 따라 초기 목표 2/3/4개, 2개 이상이면 스테이지별 최소 1개.
- [x] 기존 균형 규칙과 양립하지 않는 희귀 시드는 1개까지 낮추되 고정 시연 맵으로 폴백하지 않음.
- [x] 같은 layer 경계는 한 방향의 인접 lane만 연결하여 X자·layer 건너뛰기 방지.
- [x] 기존 경로 시퀀스 중복·위험/보상 지배·8방 길이·보스 도달 검증을 그대로 통과해야 채택.

**R2 작업**
- [x] 생성 파티션에서 대각선 후보를 수집하고 시드 기반으로 기본 2~4개 선택.
- [x] 대각선 수·스테이지 분배·인접 layer·비교차·결정적 시드 회귀 추가.
- [x] 대량 시드 생성 성공률·경로 규칙·맵/배선 회귀·프로덕션 빌드 검증.
- [x] 별도 브랜치 `codex/generated-map-diagonal-routes`·PR #307로 전달.

**PR #307 성능 리뷰 후속**
- [x] 총괄 2000시드 측정의 동기 생성 지연을 로컬 200시드로 재현: main 평균 0.22ms/최악 1.67ms, PR 평균 22.34ms/최악 106.88ms.
- [x] 동일 맵 안쪽 대각선 재배치를 최대 4회로 제한하고 `4→3→2` 하향 스윕 제거. 바깥 시도마다 `4·2·2·2`처럼 목표 하나만 선택.
- [x] 1개 희귀 폴백은 최초 유효 후보만 저장하고 바깥 맵 재시도를 모두 소진했을 때만 사용.
- [x] 50ms 성능 상한 회귀와 표본을 2000시드로 확대. 최종 재검증 평균 1.62ms·최악 14.41ms·1개 폴백 38/2000(1.9%), 경로 규칙·프리셋 미변경 통과.
- [x] `test:mapgraph`·`test:mapwiring`·프로덕션 빌드·`git diff --check` 통과.
- [x] 수정 커밋 `9e6ffcc`을 PR #307에 푸시하고 측정값과 함께 재리뷰 요청.
- [x] GitHub CI 병렬 실행에서 CPU 경쟁이 최악 시간에 섞이는 문제를 분리: `test:mapgen`만 다른 회귀가 끝난 뒤 단독 실행. 전체 `npm test` 85종 통과, 단독 구간 평균 1.59ms·최악 11.94ms.
