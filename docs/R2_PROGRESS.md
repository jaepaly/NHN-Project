...
- **최근 갱신**: 2026-07-23
- **현재 상태**: **Phase 1~5 R2 코어 전부 완료** (판정 시스템·보스 기억·시전 경제 지원·#101 소환 behavior DSL까지). → **다음 국면 = 제출물 준비**(④ AI 활용 기술 문서 = R2 오너) + 마나 경제 진화(#121)에 따른 API 재점검.
...
## ▶ 다음 할 일 (2026-07-23 · **총괄 우선순위 반영**)

> **총괄 배정 (2026-07-23)**: R2 = **#134 size·speed 지침(1순위) → #133 형상(shape)**.
> 둘 다 **판정 프롬프트(`worker.js`) 변경 = R2 도메인**이고 **클라이언트는 이미 완비**(작업 0·스키마 0). 표현력 갭("유저가 상상한 마법이 된다"는데 조용히 버려지는 수식어)을 프롬프트 몇 줄로 메우는 일.
> _(참고 — 이도원: #125 저주방 답변(병목) → #72 stage2 배경 → #128 기본탄)_

**🎯 1순위 — #134 size·speed 지침**
- [ ] 문제: 프롬프트에 `size`·`speed` **매핑 규칙이 한 줄도 없음** → LLM이 감으로 찍어 **절반만 맞음**(`거대한 불덩이`→large ✅ / `조그만 불씨`→medium ❌, `아주 빠른`→normal ❌). `큰`·`빠른`·`작은`은 플레이어 최빈 수식어.
- [ ] 수정: 프롬프트 4항 뒤에 매핑 문구 추가 (조그만/작은=small·거대한/커다란=large·하늘을 덮는=huge / 느릿·천천히=slow·빠른·번개처럼=fast). **클라 `SIZE_SCALE`·speed 이미 완전 구현** → 프롬프트 몇 줄뿐.
- [ ] 검증: `JUDGE_PROMPT_VERSION` v2.3→bump + `spell-regression` 버전핀 + **Cloudflare 재배포** + 15문장 수율 재측정(되다 안 되다 → 항상).

**2순위 — #133 형상(shape) DSL 2단계**
...
- [ ] 검증: 수율 재측정(게이트 20/20·폴백 0/10 현재 프롬프트 기준 재확인) + **모양 묘사 없을 때 shape 안 붙는지**(오탐) + 버전핀 갱신. 기준 미달이면 프롬프트만 되돌리면 무해.

**후순위 (총괄 작업 뒤)**
- [x] 제출물 ④ AI 활용 기술 문서 — 초안 1~3부 완료·리뷰 요청([PR #130](https://github.com/jaepaly/NHN-Project/pull/130)). 코멘트 오면 반영 → 심사 직전 PDF.
- [x] §6 API 재점검 — 마나 환류(#121) 반영 (이 PR 포함).
- [ ] 실뎀 로깅 검증 — "같은 속성 뎀감 과한가", 로깅 diff 백업(`scratchpad/protoscene-dmg-logging.diff`) 새 ProtoScene 재적용 후 플레이.
- [ ] 자투리: #67 §5 · #112 계약 무결성 · 열린 docs PR(#126/#127/#130) 정리.
