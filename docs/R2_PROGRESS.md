...
| **B ③77** | **런 반복 격상 — 회차 쌓일수록 "지난 답 안 통하게"** | 🎯 **진행 예정** (내 `runMemory` 도메인) |
| **C-API** | **능동형 마나(#53-5): 쿨다운 변화 → API 호출빈도·비용 영향 확인** | 🎯 **진행 예정** (§6 갱신) |
...
**▶ ③77 런 반복 격상 (신규, 나 할당)**: 회차 무관하게 런 내부가 고정(방·몹·웨이브 1회차=5회차) → 반복 동기 약함. `runMemory.ts`(clears·favoriteElement·recentDominantElements)의 격상 축이 **보스에만 갇혀 있음** → 런 전반으로 확장이 과제. 이미 깔린 코드 재사용 우선. 착수 예정.
