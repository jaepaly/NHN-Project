...
**R2 작업**
- [x] ESC 일시정지 메뉴: 전체 경로 지도 아래와 필살영창 게이지 위의 여백이 균형을 이루도록 중앙 메뉴만 120px 아래로 재배치했다. 논리 좌표 기준 지도 아래 134px, 필살영창 게이지 위 150px 여백으로 맞췄다. 최신 main rebase 뒤 전체 회귀 103종·production build·diff check와 GitHub CI test/build를 통과했으며, 총괄 병합 확인만 남았다.
