...
>
> **읽는 법**: 맨 위의 현재 상태·active 블록·현재 운영 스냅샷과 [`R2_SYSTEM_GUIDE.md`](R2_SYSTEM_GUIDE.md)가 현행 기준이다.
> 아래 Phase 1~4의 수치·모델명·체크 상태는 당시 판단을 보존한 역사 기록이다. 현재 코드나 운영값으로 복사하지 않는다.

## ▶ R2 작업 체크리스트 — 총괄 배정 #170 반영 (2026-07-24)

> **확정 배정**: 최신 원격 `origin/main`(`f602b73`)과 총괄의 [Issue #170](https://github.com/jaepaly/NHN-Project/issues/170)을 확인했다. #170은 `diawodbsdot`(임재윤)에게 배정된 **R2 공식 작업**이다.
>
> **카톡 합의 해석**: 속성 전문화 자체는 벌하지 않는다. `화염 화살 → 화염 벽 → 화염 분신`처럼 같은 속성을 다른 전술로 쓰는 것은 성장이고, **같은 문장 또는 같은 기계적 결과를 반복하는 것**만 억제한다. 문장을 다르게 말했어도 판정 결과가 완전히 같다면 반복으로 볼 수 있다.
>
> **운용 규칙**: 이 블록에는 미완료 작업만 둔다. 완료하면 여기서 지우고, 같은 변경에서 아래 **“✅ 완료 (이번 국면)”**에 작업 내용·측정값·검증·PR을 누적한다. 보류·기각은 완료로 세지 않고 의사결정 기록에 이유를 남긴다.

### P0 — 총괄 확정 배정: 진행·숙련 재설계 #170 (아래 순서 고정)

- [ ] **#170-2a 패턴 표현 계약 보강** — 48건 live Gemini 선행 감사에서 기존 `effect:element_primary:form`만으로는 화염구·화염창·화염검이 모두 `damage:fire:bolt`로 합쳐짐을 확인했다. 반대로 target·status·shape를 전부 서명에 넣으면 같은 화염벽도 말투에 따라 갈린다. 이름·power·size·speed 같은 우회 가능한 필드가 아니라 **실제 전술을 나타내는 최소 축**(우선 `delivery`/`impact` 후보)을 팀 계약으로 정하고 producer→validator→consumer를 한 번에 연결한다. **합격선**: 핵심 3종 전부 분리, 각 전술 4표현 후보 일치 ≥80%, 동일 문구 반복 ≥80%, 단일 plan 오탐 0, 2.5초 초과율 재측정.
- [ ] **#170-2b 격상·보스 기억을 원소 → 검증된 주문 패턴으로 재기준** — 2a에서 고정한 기계적 signature를 단일 주문과 시퀀스의 **순서 있는 behavior 조합**에 공통 적용하고 자동 각인·정령은 장기 습관 기록에서 제외한다. `RunMemory` 호환 마이그레이션, 격상/보스 기억 소비부, 단일·시퀀스 회귀를 만든다. **합격선**: 같은 화염이라도 전술이 다르면 약화 없음, 말만 바꾼 동일 결과는 약화, 보스 대사는 원소 톤을 유지해도 수치 판정은 pattern 기준.
- [ ] **#170-3 친화 내성 관통 계수 결정·연결** — 완료된 #170-1 기준선과 #170-2 패턴 계약이 고정된 뒤 진행한다. 친화가 해당 주문 패턴의 보스 내성을 `1.0` 방향으로 일부 되돌리되, 계수는 먼저 하드코딩하지 않고 표로 비교한다. **R2 산출물**: 순수 관통 함수·계수 후보·경계값 회귀·실측. **R1 협업**: 최종 피해 체인과 체감 승인. **합격선**: 깊게 투자한 집중형은 저항 앞에서도 무투자보다 순플러스, 낮은 친화에서는 보스 저항 정체성이 남고, #169 하한·반복 페널티와 이중 보정되지 않을 것.

> **이미 완료된 상류 조각**: 총괄 결정서 [#168](https://github.com/jaepaly/NHN-Project/pull/168), 사용 기반 친화 #166, 합산 하한 [#169](https://github.com/jaepaly/NHN-Project/pull/169). #169는 완료 항목이므로 체크리스트에 다시 넣지 않는다.

### P1 — #170 뒤에 진행할 기존 회의 제안

- [ ] **새 주문 발견 판정 + `NEW` 피드백·보상 계약** — #175로 도감이 속성×폼 인벤토리 그리드까지 발전했지만 인게임 최초 발견 신호·보상은 없다. #170의 pattern signature를 재사용해 별도 “발견 서명”이 난립하지 않게 한다. **R2 산출물**: 신규 여부 API·회귀·마나 환급 vs 일시 위력 보너스 비교안. **R3 협업**: `NEW SPELL` 팝업·도감 배지·연출.
- [ ] **융합 필살기와 `spell_plan` 연결** — 단일 이중 원소 주문의 방출과 게이지 가시성(#176)은 구현됐지만, 복합 시퀀스는 게이지만 채우고 방출하지 못한다. plan의 검증된 원소 집합으로 방출 자격을 결정하고 Power·Mana 예산 복제를 막는 계약·회귀를 만든다.

### P2 — 승인 전 제안·워치리스트

- [ ] **심사용 고급 영창 발견 동선·실 Gemini 게이트** — 기존 순환 예시를 무작정 늘리지 않고, 소환 행동·벽 형상·시퀀스·융합 조건을 첫 런 마일스톤으로 보여주는 안과 held-out 실측을 만든다.
- [ ] **연속 주문 반응 최소 계약 스파이크** — 얼음→불 같은 연속 수동 영창 반응 1~2종을 기존 히스토리로 감지하는 설계. #170의 패턴 기억과 충돌하지 않는지 먼저 확인하고 팀 승인 전 프로덕션 효과는 연결하지 않는다.
- [ ] **실플레이 시퀀스 튜닝 + #158** — 실제 입력에서 분해 누락·오버트리거·콜드스타트·form 없는 plan을 수집한다. 재현 케이스가 있을 때만 few-shot을 보강하고 held-out·2.5초 초과율을 다시 측정한다.

## 회의 업그레이드 아이디어 전체 맵

| 회의 아이디어 | 최신 `origin/main` 상태 | 실제 남은 업그레이드 | 주도 후보 |
|---|---|---|---|
| 속성 벌 제거·같은 주문 반복 억제 | #168 결정서·#169 합산 하한은 머지됐지만 격상·보스 내성은 아직 원소 기준. 48건 감사에서 기존 `effect:element:form`의 공격 전술 충돌 확인 | 최소 전술 축 보강 → 단일·시퀀스 signature → RunMemory/보스 소비부 전환 | **R2 확정 배정 #170** |
| 친화 내성 관통 | 사용 친화·연속 VFX·경험치 바까지 구현됐지만 내성 관통은 없음 | 실측 후 관통 계수와 최종 피해 체인 연결 | **R2 확정 배정 #170 + R1 승인** |
| 주문 발견 도감·새 마법 정보 | #175에서 영구 도감을 속성×폼 인벤토리 그리드로 확장 | 인게임 `NEW` 피드백, #170과 공유하는 기계적 발견 signature, 발견 보상 | **R2 계약 + R3 UX** |
| 발견 보너스 | 미구현. 다양성 보너스는 “최근과 다름”이지 “최초 발견”이 아님 | 마나 환급/일시 위력 중 하나를 악용 방지 게이트와 함께 선택 | **R2 설계·계측**, 총괄 결정 |
| 도파민용 필살기 | 융합 게이지·단일 이중 원소 방출·게이지 HUD 승격(#176) 구현 | `spell_plan` 방출 지원·실전 페이스 검증 | **R2 계약 + 총괄/R1 연출·전투** |
| 시퀀스·고급 주문 온보딩 | 영창 바 예시 13개에 시퀀스·소환 행동·벽 형상 포함 | 짧은 심사에서 실제로 보게 하는 마일스톤형 힌트와 실 Gemini 수율 검증 | **R2 검증 + R3 UX** |
| 주문 간 시너지·원소 반응 | 다양성 보너스와 동시 이중 원소 융합은 있으나 연속 시전 반응은 없음 | 1~2개 최소 반응으로 재미/복잡도 스파이크 후 확대 판단 | **R2 감지 계약 + R1/R3 효과** |
| 도감에서 최대 6개 장착 | #165에 “유산 로드아웃 방향 C” 제안서가 머지됐지만 구현 확정은 아님 | 자유 영창을 고정 덱으로 대체하지 않는 탈부착 범위 결정 | **총괄 결정**, R2는 판정 데이터 자문 |
| 현재 증강·효과 확인 탭 | #164로 런 보상 요약·보유 배지가 추가됨 | 상세 수치·방 저주·전체 획득 이력이 정말 더 필요한지 플레이테스트 | **총괄/R3 UX** |
| 각인 쿨다운 감소·자동 시전 강화 | 각인 강화와 자동 시전 주기 감소가 이미 있음 | 자동화가 수동 영창 동기를 먹지 않도록 필살기·발견 보상과 함께 계측 | **총괄/R3 밸런스**, R2 로그 지원 |

- **담당**: 임재윤 (R2 — 판정 프롬프트·프록시 인프라·캐싱/폴백·보스 기억)
- **최근 갱신**: 2026-07-24
- **현재 상태**: **판정 코어 + 표현력 DSL(#101 소환·#133 형상·영창 시퀀스) 완료·배포·머지.** 원격 최신 `main`에는 사용 친화(#166)·숙련 재설계 결정서(#168)·합산 하한(#169)·친화 피드백(#173)·도감 인벤토리(#175)·융합 게이지 HUD(#176)·런 HUD 정리(#177)가 반영됐다. **#170-1 계측 정합화와 #170-2 선행 표현 감사는 완료**했고, 감사 결과에 따라 남은 순서는 **최소 전술 축 계약(#170-2a) → 패턴 재기준(#170-2b) → 친화 관통(#170-3)**이다.
...
### 현재 운영 스냅샷 (2026-07-24)

| 항목 | 현행 |
|---|---|
| 모델 | `gemini-3.5-flash-lite` 안정 버전 명시 핀 (`-latest` 금지) |
| 판정 계약 | `schema_version: 2`, 단일 `spell` 또는 복합 `spell_plan` |
| 프롬프트·캐시 | `meaning-v2.6-seq`, `incant:judge:v2:meaning-v2.6-seq:` |
| 지연·폴백 | 2.5초 타임아웃, 실패·무효·원격 fizzle은 MockJudge로 복구 |
| Worker 보호막 | IP·Worker 인스턴스별 15 RPM. Gemini 프로젝트 활성 한도와는 별개 |
| 반복·다양성 | 같은 문장 ×0.9, 같은 스펙 변주 ×0.95, 하한 ×0.6, 다양성 최대 +30% |
| 시전 경제 | 마나 재생 2/s, 주문 킬 +6, 감쇠 시전 바닥 5, 입력락 0.4s(하한 0.15s) |

## 최근 의사결정·완료 맥락 (2026-07-23~24)
...
- [x] **#170-2 선행 — 자유 영창 표현·전술 충돌 live Gemini 감사** — 엔진을 바꾸기 전에 6개 화염 전술×4표현(기술명·명령문·서술문·영문)×N=2, 총 **48건**을 Worker 직접 호출(4.5초 페이싱, 캐시·Mock 없음)했다. 유효 Gemini **48/48**, plan 오탐 **0/48**, 2.5초 초과 **0/48**(p50 1.598s·p90 1.869s·max 2.301s). 후보 `effect:element:form`의 같은 의도 평균 일치는 93.8%, 동일 문구 반복은 95.8%였지만 화염벽은 75%로 effect가 흔들렸고, **화염구·화염창·화염검이 모두 `damage:fire:bolt`로 충돌**했다. target·status·shape까지 포함한 상세 fingerprint도 세 전술을 분리하지 못했고 반복 안정성은 70.8%였다. 결론: 기존 signature로 엔진을 먼저 바꾸지 않고 최소 전술 축을 보강한 뒤 재측정한다. 산출물: `scripts/spell-expression-audit.ts`, `docs/SPELL_EXPRESSION_AUDIT.{md,json}`, `npm run audit:expression`. 감사 도구 재번들·JSON 파싱, 등록된 전체 `test:*` **44개**, TypeScript·프로덕션 빌드 통과.
- [x] **#170-1 실제 피해 계측 정합화 + 기준선 측정** — 전투 피해 경로는 변경하지 않고 DEV `dmg` 로그가 엔진과 동일한 `flooredResistMultiplier`를 사용하도록 `damageTelemetry.ts` 순수 계측 함수를 추가했다. 기존 `bossResist`는 호환용 원시값으로 유지하고 `rawBossResist·appliedBossMultiplier·combinedDebuff`를 추가, `finalVsBoss`를 실제 엔진 공식으로 교정했다. 고정 행렬 결과: 내성만 **30→50**, 격상0.4+내성0.3 **12→50**, 반복하한0.6까지 중첩 **7→30**, 하한 위 **72→72**. `test:dmgtelemetry`·`test:debufffloor`·`test:history`·`test:boss`·`test:runmemory`와 프로덕션 빌드 통과. **게임 밸런스·피해 계산 변화 없음.**
- [x] **2026-07-24 전체 정합성 최신화** — 최신 main의 영창 시퀀스·융합 게이지까지 코드와 README/GDD/R2·제출·프록시 문서를 전수 대조. 현재값(`gemini-3.5-flash-lite`, `meaning-v2.6-seq`, Worker 15 RPM 보호막, 반복 ×0.9/×0.95·하한 ×0.6, 마나·입력락, 복합 plan-only)을 상단 기준으로 통일하고 옛 Phase 수치는 역사 기록으로 표시. `AI_USAGE_TECH.pdf` 9쪽 재생성·전 페이지 시각 검수, 전체 `test:*` 39개·프로덕션 빌드 통과.
...
| **② 히스토리** | cast+발동 확정 주문만 런 공책에 기록 | 같은 문장 ×0.9·같은 스펙 변주 ×0.95, 하한 ×0.6 + 다양성 보너스 · `bossMemory()` 요약 |
| **③ 보스 기억** | 공책·지난 런으로 보스가 대비 | 단기 내성 ×0.3(이번 런 최다) · 장기 부분 내성(최근 5런·최다 1개 — 과잉내성 방지) · 도발 대사(`/boss-line`+템플릿 폴백) |
| **④ 영창 시퀀스** | 복합 문장을 순차·동시 동작으로 실행 | `spell_plan` 검증·전체 예산 재분배 · plan 단위 마나/기록 · 단계별 원소·폼을 보스 기억·저주·도감에 반영 |

fizzle/blocked는 마나·영창 입력락·기록 모두 노카운트. 판정~대사 전부 "실패해도 게임은 멈추지 않는다" 원칙.
...
> 아래 세부 일지는 당시 진행 순서를 보존한다. 현재 운영값은 문서 상단 스냅샷을 따른다.
...
**▶ C-API 확인 (#53-5, 당시 과제·완료)**: C안으로 시전 케이던스가 빨라질 때 호출 빈도와 당시 조사한 쿼터의 상호작용을 검토해 SUBMISSION_PLAN §6에 반영했다. 현재는 고정 RPD를 가정하지 않고 Worker 15 RPM 보호막과 AI Studio의 프로젝트 활성 한도를 구분해 운영한다.
...
> 이 섹션은 Phase 4 당시 기록이다. 모델·가격·쿼터는 상단 운영 스냅샷과 `SUBMISSION_PLAN §6`을 따른다.
...
| C | **당시 심사 기간 할당량 정책 초안** — 당시 조사값(무료 15RPM/1000RPD)·호출 프로필·대응 4단·비용($0.00014/콜) → SUBMISSION_PLAN §6 | ✅ 당시 완료 (현재 운영값 아님. 최신 정책은 상단·SUBMISSION_PLAN §6 참고) |
...
> 아래는 Phase 3 당시 계약·통합 기록이다. 현행 구조는 상단과 `R2_SYSTEM_GUIDE.md`를 따른다.
...
> 아래 반복 수치는 PR #13 당시 초기값이다. 현행은 같은 문장 ×0.9·같은 스펙 변주 ×0.95·하한 ×0.6이다.
...
| P0-c | 반복 패널티 초기안 (당시 동일 문장 power×0.8, floor 0.3, 로컬 강제) | ✅ 당시 완료 (후속 #92에서 현행값으로 완화) |
...
- 당시 `repeatMultiplier(text)` 초기안은 **처음 1.0 → 재사용부터 ×0.8 누적, 하한 0.3**이었다. 이후 #92에서 현행 ×0.9·유사 스펙 ×0.95·하한 ×0.6으로 완화됐다.
...
> 아래는 초기 프록시 구축 당시의 모델·할당량 사고 기록이다. 당시 별칭과 무료 한도는 현재 운영 지침이 아니며, 현행은 상단 스냅샷을 따른다.
