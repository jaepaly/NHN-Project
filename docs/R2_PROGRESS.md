...
| # | 작업 | 상태 |
|---|---|---|
| B ①53 | 마나 제거 + 위력 비례 쿨다운 프로토타입 | ✅ 프로토타입·검증·리뷰응답 (PR #60) → **팀 A/B 결정 대기** |
| B ②56 | 조준 타겟팅 (해석 불일치 UX) | ✅ R2 리뷰 완료 (총괄 프로토타입 PR #55) |
| 도구 | 플레이 판정 로거 (`logs/play.jsonl`, 피드백용) | ✅ 머지 (PR #41) |
| 인프라 | 프록시 CORS localhost 임의 포트 (Mock 조용한 폴백 방지) | ✅ 머지 (PR #40) |

**▶ ①53 위력쿨다운 현황**: `castCooldownFromPower`(1.2+power/100×2.4)·`startGlobalCooldown(power?)` 하위호환·`VITE_CAST_ECONOMY` 플래그(기본 mana). 라이브 검증(운석 위력95→3.48s 공식 일치). 총괄 리뷰 **품질 통과**, 채택은 손맛 A/B 게이트(총괄+이도원). **R2 입장 = A(쿨다운 단순화) 지지** — 마나식은 창의적 고위력 주문을 과세해 "말이 곧 마법" 정체성과 충돌; B(자원 심화)는 마나 재설계 필요해 Phase 5(3주) 일정 과함. **A 확정 시 R2 후속**: 보상 풀 이관(마나 격류·MAX MANA→신속영창 계열), 오토 ≤40% 게이트 재측정.
