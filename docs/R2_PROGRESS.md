...
- [ ] 4단계: HP·마나·실드를 우측 하단 빌드 칩 왼쪽으로 옮겨 축약하고 저체력 경고·현재값/최대값을 보존한다.
- [ ] 5단계: 8원소 친화 정보량을 유지하며 패널·여백·장식을 축소하고 화면 크기별 안전 배치를 적용한다.
- [ ] 전용 순수 기하/모델 회귀와 기존 HUD·미니맵·보스 저항 회귀, 타입 검사·프로덕션 빌드·실화면을 확인한다.
