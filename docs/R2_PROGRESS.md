...
- **최근 갱신**: 2026-07-23
- **현재 상태**: **판정 코어 + 표현력 DSL(#101 소환 behavior·#133 형상 shape) 전부 완료·배포·머지.** #134 size·speed는 **팬텀**(실 Gemini 40/40 정확)으로 종결, 진짜 결함이던 Mock 수식어 맹인은 총괄 #138로 수정. 제출물 ④ AI 활용 기술 문서 **머지됨**(총괄 리뷰 통과). → **다음 = ④ PDF 추출(제출 형태·팀 전달) + 자투리.**
...
**🎯 다음 — 제출물 마무리 (판정 코어·#133·#134 전부 종결)**
- [x] 제출물 ④ AI 활용 기술 문서 — 초안 1~3부 완료·**총괄 리뷰 통과·머지**([PR #130](https://github.com/jaepaly/NHN-Project/pull/130)). 총괄이 코드와 한 글자까지 대조 확인, 프롬프트 버전 표기만 정정 푸시.
- [ ] **🎯 ④ PDF 변환** (제출 형태·팀원 전달) — 마크다운은 main에 있으나 **제출물은 PDF**라 아직 팀 전달분 없음. 총괄 선택 제안 2건(1.5 held-out 실측 수치·2.1 Gemini 판정 각주) 반영 후 PDF 추출.
