...
- [~] 4단계: HP·마나·실드를 210×65px로 축약해 우측 하단 빌드 칩 왼쪽 12px 간격에 옮기고 현재값/최대값, 열파 표시, 쿨다운 띠와 HP 30% 이하 `!` 적색 경고를 보존했다. 실플레이 피드백에 따라 30% 이하 진입·35% 이상 해제되는 단일 저체력 적색 맥동을 추가했으며 전용 9군·함정·열파·컴팩트 HUD·UI 팔레트 회귀와 production build를 통과했다. 실화면 강도 확인 대기.
