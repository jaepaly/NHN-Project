...
- [~] 최신 `main` rebase와 로컬 회귀·빌드는 완료했다. 브랜치 푸시 뒤 GitHub CI와 총괄의 병합 가능 상태를 다시 확인한다.

**R2 작업**
- [x] ESC 일시정지 메뉴: 전체 경로 지도 아래와 필살영창 게이지 위의 여백이 균형을 이루도록 중앙 메뉴만 120px 아래로 재배치했다. 논리 좌표 기준 지도 아래 134px, 필살영창 게이지 위 150px 여백으로 맞췄다. 최신 main rebase 뒤 전체 회귀 103종·production build·diff check를 다시 통과했으며, PR #347의 새 CI·총괄 병합 확인만 남았다.
...
- [~] 실제 플레이로 일반방·보스방·ESC 검사 화면의 겹침과 가독성을 최종 확인한 뒤, PR #347 새 CI와 총괄의 병합 가능 상태를 확인한다.
