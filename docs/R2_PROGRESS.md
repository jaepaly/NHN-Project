...
- [ ] **#170-1 실제 피해 계측 정합화 + 기준선 측정** — #148의 `dmg` 로깅은 이미 연결되어 있으므로 옛 scratchpad 패치를 다시 적용하지 않는다. 다만 #169가 `격상 × 보스 내성` 합산 하한 ×0.5를 적용한 뒤에도 로그의 `finalVsBoss`는 원시 `bossResist`를 곱해 계산하므로 실제 피해와 어긋날 수 있다. 먼저 로그를 `rawBossResist / appliedBossResist / combinedDebuff / actualFinalVsBoss` 기준으로 맞춘 뒤, ① 같은 문장 반복 ② 다른 문장·같은 `effect:element:form` ③ 같은 원소·다른 폼/효과 ④ 다른 원소·같은 폼을 고정 seed로 측정한다. **산출물**: 표와 `logs/play.jsonl` 근거, #170 코멘트, 회귀. **합격선**: “같은 속성이라서”만 발생하는 손해와 “같은 주문 반복” 손해가 분리되어 보일 것.
...
- [ ] **#170-3 친화 내성 관통 계수 결정·연결** — #170-1 기준선과 #170-2 패턴 계약이 고정된 뒤 진행한다. 친화가 해당 주문 패턴의 보스 내성을 `1.0` 방향으로 일부 되돌리되, 계수는 먼저 하드코딩하지 않고 표로 비교한다. **R2 산출물**: 순수 관통 함수·계수 후보·경계값 회귀·실측. **R1 협업**: 최종 피해 체인과 체감 승인. **합격선**: 깊게 투자한 집중형은 저항 앞에서도 무투자보다 순플러스, 낮은 친화에서는 보스 저항 정체성이 남고, #169 하한·반복 페널티와 이중 보정되지 않을 것.
...
- **최근 갱신**: 2026-07-24
- **현재 상태**: **판정 코어 + 표현력 DSL(#101 소환·#133 형상·영창 시퀀스) 완료·배포·머지.** 원격 최신 `main`에는 사용 친화(#166)·숙련 재설계 결정서(#168)·합산 하한(#169)·친화 피드백(#173)·도감 인벤토리(#175)가 추가됐다. **새 확정 R2 작업은 #170의 측정→패턴 재기준→친화 관통 3단계**이며, 이전 회의 제안은 그 아래 우선순위로 내렸다.
