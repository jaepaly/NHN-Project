...
## ☐ 지금 작업 흐름 — 다음 방 선택 W/S 조작 일관성 (2026-07-31)

> **목표**: WASD로 이동하는 플레이 흐름과 다음 방 선택 UI의 키보드 조작을 통일한다. 화면 안내와 포커스 이동은 `W/S + Enter`로 통일하고, 숫자키·마우스 직접 선택은 보존한다.
...
- [x] 최신 `main` 기반 PR #306으로 전달하고 #305 후속 통합 시 W/S 입력 보존 관계 명시.
