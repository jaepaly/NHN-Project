...
> **확정 배정**: 최신 원격 `origin/main`(`e2998eb`)과 총괄의 [Issue #170](https://github.com/jaepaly/NHN-Project/issues/170)을 확인했다. #170은 `diawodbsdot`(임재윤)에게 배정된 **R2 공식 작업**이다.
>
...
- [ ] **#170-2 격상·보스 기억을 원소 → 주문 패턴으로 재기준** — 새 LLM 필드를 만들지 않고, 단일 주문은 이미 반복 판정에 쓰는 `spellSignature(effect:element_primary:form)`를 1차 기준으로 삼는다. 주문명만 쓰면 이름 변주로 우회되고, 폼만 쓰면 서로 다른 원소·효과까지 한꺼번에 벌하므로 제외한다. 시퀀스는 실행된 behavior signature의 **순서 있는 조합**으로 정의하고 자동 각인·정령은 장기 습관 기록에서 제외한다. **산출물**: pattern 계약, `RunMemory` 호환 마이그레이션, 격상/보스 기억 소비부, 단일·시퀀스 회귀. **합격선**: 같은 화염이라도 폼/효과가 다르면 약화 없음, 말만 바꾼 동일 결과는 약화, 보스 대사는 원소 톤을 유지해도 수치 판정은 pattern 기준.
...
- [ ] **융합 필살기와 `spell_plan` 연결** — 단일 이중 원소 주문의 방출은 구현됐고 게이지 가시성은 총괄 PR #176에서 진행 중이지만, 복합 시퀀스는 게이지만 채우고 방출하지 못한다. plan의 검증된 원소 집합으로 방출 자격을 결정하고 Power·Mana 예산 복제를 막는 계약·회귀를 만든다.
...
|---|---|---|---|
| 속성 벌 제거·같은 주문 반복 억제 | #168 결정서·#169 합산 하한은 머지됐지만 격상·보스 내성은 아직 원소 기준 | `effect:element:form` 패턴 계약, 시퀀스 서명, RunMemory/보스 소비부 전환 | **R2 확정 배정 #170** |
...
| 도파민용 필살기 | 융합 게이지·단일 이중 원소 방출 구현, 게이지 HUD 승격은 총괄 PR #176 진행 중 | `spell_plan` 방출 지원·실전 페이스 검증 | **R2 계약 + 총괄/R1 연출·전투** |
...
- **최근 갱신**: 2026-07-24
- **현재 상태**: **판정 코어 + 표현력 DSL(#101 소환·#133 형상·영창 시퀀스) 완료·배포·머지.** 원격 최신 `main`에는 사용 친화(#166)·숙련 재설계 결정서(#168)·합산 하한(#169)·친화 피드백(#173)·도감 인벤토리(#175)가 추가됐다. **#170-1 계측 정합화는 완료**했고, 남은 확정 순서는 **#170-2 패턴 재기준 → #170-3 친화 관통**이다. 이전 회의 제안은 그 아래 우선순위로 유지한다.
