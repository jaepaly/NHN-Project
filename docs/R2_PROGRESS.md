...
## ☐ 지금 작업 흐름 — #239 바닥형 지형 Step 3~5 (2026-07-29)

> **담당 확정**: R2(임재윤)가 #237에서 만든 용암·독지대 기전의 후속인 Step 3~5를 계속 맡는다. 인접 개발까지 같은 담당자가 마무리해 팀원의 동시 작업 부담과 경계 비용을 줄인다. 프롬프트 판정·지연 최적화는 **별도 채팅·별도 브랜치**에서 진행 중이므로 이 체크리스트와 PR에 섞지 않는다.

**범위·의존성**
- [x] #239 담당을 R2로 재확인하고 프롬프트 작업과 작업 흐름 분리
- [ ] 보존 브랜치 `codex/floor-hazard-step3-5`와 최신 `main`의 차이 감사 — 그대로 합치지 않고 현재 계약에 맞는 부분만 이식
- [ ] #239의 기존 설계 4답과 R3 축소 제안을 비교해 최종 구현 경계 기록 — 기본안은 Step 3~5 전체, HUD·중첩 규칙은 최소 명시
- [~] #264 도착점 keep-out은 R1 프리셋이 `MapNode.terrain`을 채운 뒤 R2 배치 소비 부분 검토
- [~] #214 선택 노드↔실제 조우 본배선은 R3 통합 결과 대기

**구현 순서 — 한 단계씩 검증**
- [ ] **Step 3 플레이어 디버프** — 독지대 약화(`damageOutMultiplier`)·둔화(`moveSpeedMultiplier`)를 기존 buff 상태·만료 루프에 최소 확장. `damageInMultiplier`와 `debuffFloor.ts`는 건드리지 않음
- [ ] Step 3 회귀·타입 검사·빌드 후 중간 검토
- [ ] **Step 4 잔류 도트** — 독지대 이탈 뒤 `lingerSeconds` 동안 기존 스케일 게임시간 틱에서 지속, 재진입·방 종료·일시정지 불변식 검증
- [ ] Step 4 회귀·타입 검사·빌드 후 중간 검토
- [ ] **Step 5 카운터 정화** — 영창 원소/효과로 해제·단축·지형 면역만 허용. 비상성 원소 감산·패널티 없음, 방당 횟수와 면역 시간을 실제 동작으로 검증
- [ ] Step 5 회귀·전체 테스트·프로덕션 빌드
- [ ] fixture가 아닌 실플레이 로그에서 디버프·잔류·정화·면역 동작 확인
- [ ] AI 활용 로그 갱신, 별도 기능 브랜치 푸시, #239 연결 PR 및 총괄 리뷰 요청

**병렬 작업에서 제외**
- [~] 프롬프트 판정 품질·응답시간·compact/sparse 출력 실험 — 다른 채팅의 전용 브랜치에서 계속하며 #239 변경에 포함하지 않음
- [~] PR #268 실플레이 판정 로그 — 충돌 해소·CI 통과, 총괄 승인·머지 대기
- [~] PR #261 과거 로그 백필 — #268 머지 뒤 고유 기록만 비교하고 중복이면 정리
