...
> **목표**: WASD로 이동하는 플레이 흐름과 다음 방 선택 UI의 키보드 조작을 통일한다. 화면 안내는 `W/S + Enter`를 주 입력으로 표시하고, 기존 방향키·숫자키·마우스 입력은 보조 수단으로 보존한다.
...
- [x] 화면 안내를 `W/S + Enter` 중심으로 수정하되 방향키·숫자키·클릭 호환 유지.
- [x] W/S 방향·한글 IME 상태·기존 방향키 회귀 검증. `test:roomchoice` 6군·프로덕션 빌드·`git diff --check` 통과.
- [ ] 최신 `main` 기반 별도 PR로 전달하고 #305 후속 통합 관계 명시.
