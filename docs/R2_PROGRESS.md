...
| # | 작업 | 상태 |
|---|---|---|
| A | **판정 품질 튜닝** — power 산정 일관성·창의성 가점 프롬프트 개선. `snapshot:quality` 전/후 비교(제출물 ④ 사례). 변경 시 캐시 `promptVersion` ↑ | ⬜ |
| B | **`/evolve-name` 클라이언트 캐시** — 총괄 ④ 소비 확정(런당 진화·융합 각 1~2회). localStorage 키 `kind+elements(+baseName)`, 판정 캐시와 같은 버전 접두사 | ⬜ |
| C | **심사 기간 할당량 정책 초안** — 무료 티어(RPD 500) 소진 시나리오·키 스왑/유료 전환 옵션·비용 추정 → SUBMISSION_PLAN 갱신 제안 | ⬜ |
| D | 여유 시: 보스 대사·작명 다양화 (temperature·프롬프트 변형) | ⬜ |

**▶ 현재 위치**: Phase 4 착수 (`feat/phase4-r2`, 최신 main 기준). Phase 3 ①~⑤ 전부 main 머지 완료. 캐시(B)는 총괄이 소비 방식 확정 → 백로그에서 실작업으로 승격.
