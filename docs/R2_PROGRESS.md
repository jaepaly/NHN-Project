...
| # | 작업 | 상태 |
|---|---|---|
| ① | 내성 프로필 모듈 (`bossMemory`→최다원소 저항 ×0.3·최다폼 카운터, 순수함수+테스트) | ⬜ **← 다음 할 일** |
| ② | 런 간 기억 (localStorage, `incant:runmemory:v1:` 버전 접두사) | ⬜ |
...
**▶ 다음 할 일**: **① 내성 프로필 모듈**부터. 총괄 보스 코어가 이걸 소비하니 **계약 우선**(§5 병합 순서). Phase 2의 `BossMemoryProfile` 초안을 확장. 개발은 Mock+템플릿 폴백(라이브 Gemini는 대사 테스트 최소만).

**경계**: 보스 전투 코어·연출·통합 QA는 **총괄**. R2는 기억·내성·대사 **모듈+계약**까지. `SpellSpec`/`RunContract` 변경 없음 예정.

### ① 내성 프로필 모듈 — ⬜ (다음)

- `bossMemory()` 요약 → **최다 원소 저항**(그 원소 피해 ×0.3) + **최다 폼 카운터 유형** 결정.
- **순수 함수 + 회귀 스크립트** (Phase 2 히스토리와 같은 방식). 전투 적용은 총괄 보스 코어가 소비.
...
- 프록시 `/boss-line` 엔드포인트: 런 요약 → 1~2문장 위협 대사.
- **폴백 템플릿 대사 필수** (프록시 실패·타임아웃에도 보스는 말한다). 대사도 검증·길이 제한.
...
- 진화·융합 작명 엔드포인트, 폴백 템플릿 필수 ([PROGRESSION_DESIGN.md](PROGRESSION_DESIGN.md) §5). 성장 시스템용, 보스 기억 이후.
