...
## 현재 작업 — 제출물 ④ AI 기술 문서 정정·제출 스크린샷 준비 (#362, 2026-08-09)
...
- [x] 최신 `origin/main`(`ed718e4`)에서 전용 브랜치 `codex/submission-ai-tech-screenshots`를 분리했다.
- [x] #362의 `move` DSL 삭제 정정 5곳과 PR #390의 프롬프트 `v2.15 → v2.31` 갱신 요청을 확인했다.
- [x] 제출물 ③용 7컷 목록과 production·1080p·DEV 표식 제외 조건을 확인했다.

**R2 작업**

- [x] 최신 Worker·클라이언트 판정 계약을 코드와 대조해 `AI_USAGE_TECH.md`의 구조·프롬프트·수치·대표 사례를 정정했다.
- [x] 기술 문서 PDF를 재생성하고 9페이지의 한글·표·코드 블록 렌더링을 검증했다.
- [x] production 촬영 환경에서 영창·창의 주문·자기 강화·form/wait·보상 3택 원본 5장을 확보했다.
- [x] 미확보 보스 컷과 선택 친화도 비교 컷의 정상 플레이 재현·촬영 절차를 `submission-screenshots/README.md`에 기록했다.
- [ ] `git diff --check`와 문서/PDF 검증 후 AI 사용 로그를 갱신하고 PR로 전달한다.
