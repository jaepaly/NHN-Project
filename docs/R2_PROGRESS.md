...
- [ ] **① 소스↔배포 정합 (⚠️ blocked)** — production=v2.12인데 `proxy/worker.js`(main)=v2.6(destination 5종). **v2.12 진짜 소스가 git 어디에도 없음**(노트북 미커밋 또는 배포본에만). → 노트북에서 worker.js 커밋·푸시해야 진행. 이후 main 반영 + 버전핀 3곳 v2.12 + 회귀+예행연습.
