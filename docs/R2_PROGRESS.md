...
| ⑤ | `/evolve-name` 엔드포인트 (Phase 3.5, 후순위) | 🔧 진행 중 (계약+폴백·클라이언트 ✅ / 프록시·테스트 대기) |

> **①~④는 총괄이 보스에 통합·라이브 검증 완료** (`a02046c` — 장기 fire×0.6 부분내성 + 기억 대사 실측). R2 Phase 3 핵심은 게임에 살아있음. **남은 건 ⑤(성장 시스템용, ~7/24)뿐.**

**▶ 현재 위치**: **R2 Phase 3 핵심(①~④) 완료·머지·게임 통합 검증됨** (PR #22 머지, 총괄 `a02046c` 통합). 진행 중: ⑤ `/evolve-name` (`feat/evolve-name` 브랜치, Step 2/5까지).
...
**▶ 다음 할 일**: ⑤ `/evolve-name` 마무리 — Step 3(프록시 라우트) → Step 4(배포) → Step 5(테스트·PR). 성장 시스템 진화·융합(~7/24)이 소비.
...
### ⑤ `/evolve-name` (Phase 3.5) — 🔧 진행 중 (~7/24 필요, `feat/evolve-name`)
...
- [ ] **Step 3 프록시 라우트** — `worker.js`에 `/evolve-name` + 작명 프롬프트.
- [ ] **Step 4 배포 + 라이브 확인**.
- [ ] **Step 5 회귀 테스트 + 커밋 + PR**.
