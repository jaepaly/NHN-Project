...
같은 문장이 timeout 직후 재호출에서는 2초 안에 끝났으므로 프롬프트별 고정 지연이나 할당량 소진으로 보기 어렵다. 현재 증거에 가장 잘 맞는 설명은 정상 응답이 약 1.5초에 모이지만 간헐적인 Worker→Gemini→응답 경로 tail이 클라이언트의 2.5/3.2초 hard cutoff를 넘는 현상이다.

현재 DEV 로그는 클라이언트가 언제 포기했는지만 알 수 있고, 시간이 Worker 진입 전·Gemini upstream·응답 전송 중 어디에서 소모됐는지는 알 수 없다. 따라서 프롬프트 축소나 timeout 확대보다 먼저 다음 관측을 추가해야 한다.

1. 클라이언트와 Worker에 공통 request ID를 붙인다.
2. Worker에서 전체 처리 시간과 Gemini upstream 시간을 분리 기록한다.
3. 클라이언트 로그에 실제 적용된 timeout budget과 `looksSequential` 결과를 남긴다.
