...
- `sparse-plan-r3` Version `cc4231c3-0788-42da-8d06-a62241ad5cd9`
  - 이동+공격은 유지했지만 r2와 같은 두 입력을 다시 single로 분류했고, 반복 입력에는 무단 heal도 추가했다.
  - 해당 5+5콜 표본에서 candidate response 중앙값은 549→343B(-37.5%)였지만 p50은 1,506→1,567ms(+61ms)로 빨라지지 않았다.
  - **최종 NO-GO**: 품질 스모크를 통과하지 못했으므로 42종 품질·36콜 지연 집중 측정, PR, production 배포를 진행하지 않는다.
