...
| `sfx-boss-pattern-warning.wav` | `boss-hazard-spawn_re_a.wav` (재설계 Prompt A, Duration 2초) | `dark fantasy boss hazard zones forming across the arena floor, several magical areas emerge gradually and spread outward over time with a continuous textured arcane motion, the field becomes fully established and then gently settles, environmental and spatial rather than percussive, clear medium-frequency presence with a soft sustained body, no initial hit, no crack, no pulse, no activation lock, no impact, no explosion, no elemental identity, no deep rumble, no dissonant chord, no voice, no music, 1.15 seconds` | 위험 장판 전용 후보로 채택했으나 인게임에서 장판 형성음보다 패턴 경고음처럼 들린다는 사용자 판단에 따라 공통 경고음으로 역할을 변경했다. 후처리 1.835초·피크 -7dBFS이며 위험 장판, 돌진, 탄막, 미러 캐스트의 예고 시작에 한 번 재생한다. 각 패턴의 실행음은 별도로 유지한다. |
