...
| **영창 시퀀스 (복합)** | `worker.js` 프롬프트 **7항 + few-shot 예시** (v2.12: move `custom-vector`+`angle` 방향 이동 — 왼쪽 -90/오른쪽 90/비스듬 ±45 · **v2.15: 추상·시적 영창명도 시퀀스로 확장** #209) | `spellPlanValidate.ts` `validateSpellPlan` | `ProtoScene.runSequenceCast` · `sequencePlan.ts` (오버랩 `sequenceFlowTimeline` #210 · 게이트 `npm run gate:sequence`) |
...
- **단일소스 사슬**: `types.ts:15` `FORMS`(**13종**: bolt·beam·**slash**·wave·nova·rain·wall·cage·orbit·summon·buff·zone·chain) → `validate.ts:34` `isOneOf(FORMS, …)`(FORMS 재사용 = 검증 자동) → `spellRenderer.ts` `switch(spec.form)` → `palette.ts` `FORM_LABELS`. **exhaustive `Record<SpellForm>` 3개**(palette·mockJudge·codexOverlay)도 slash 넣으면 타입에러로 누락 강제 → 라벨/아이콘 빠뜨릴 위험 없음.
...
- **엔진 렌더 실태**: `bolt·beam·slash·wave·nova·zone·rain·chain·cage`는 각자 렌더 함수(`slash`=총괄 `castSlash`, 적 위치 발현 #193). **`wall/orbit/summon/buff`는 switch에 없어 `bolt`로 폴백**(이들은 persistentFormConfig·behaviorRunner 등 별 시스템서 처리). `speed`가 투사체 속도를 결정 → **같은 form도 speed로 화면에서 갈린다**(예: 화염구 bolt/normal vs 화염창 bolt/fast).
- **✅ 근접 참격 = slash로 해소됨(v2.14)**: 예전엔 근접(검·도끼·발톱)이 form 앵커 부재로 75% bolt(투사체)로 뭉갰다(실측 #188). → `slash` form 신설(#189→#191 DSL) + `castSlash` 렌더(총괄 #190/192/193)로 해결. 프롬프트 앵커="베거나 휘두르는 동작→slash, bolt/beam과 구분". 실측: 근접 100% slash, 비근접 잠식 0. **⚠️ 배선 교훈**: DSL만 넣고 인게임 시퀀스 렌더를 실측 안 하면 융합 경로가 bolt로 샐 수 있다(총괄 #192가 이 배선 복구).
- **form 하나 추가 = 정확히 4곳(+버전핀 3) — slash가 실제 사례(#191)**: ① `types.ts` `FORMS` + exhaustive Record 3개 ② `worker.js` 프롬프트 3단계 앵커 + 스키마 목록 ③ `spellRenderer.ts` `switch` case + 렌더 함수(총괄 소관) ④ 버전핀 3곳. ⚠️ **"특정 단어를 규칙으로 강제"(예: 검→beam)는 ②만 건드리는 일관성 패칭 = 과적합 함정(#170/§7).** 확장은 **개념 단위 새 form + held-out + 엔진 소비자**로. **인게임 렌더(단일+시퀀스 경로 둘 다) 실측 필수.**
...
- **`judge-output.js` = 좁은 정규화, repair 아님** — 모델이 `delivery`(mechanic enum)를 `form` 칸에 복사한 교차 enum 누출만(`form===delivery`일 때) 교정. 그 외 알 수 없는 form은 손대지 않고 클라 검증기가 거부. **현행 프롬프트는 `mechanic`을 출력하지 않아 지금은 휴면**(#170 실험 잔재 안전망). 의미 복구·연쇄 계층 아님 → §7 경계 대상 아님.
- **이동 판정 = 화면 절대 + 융합 인식 (v2.14)** — ① 방향(#187): `custom-vector` angle은 **화면 절대**(위=0 시계방향), 표적 무관. 엔진 `screenDirectionFromAngle`(`{sinθ,−cosθ}`). ② 융합(#191): 예전엔 시퀀스 트리거가 **명시적 순차("A한 뒤 B")에만** 걸려 융합 이동("파고들어 벤다")을 29%만 잡았다 → 7단계에 "위치이동 동사는 -며/-어 융합돼도 move" 넛지(29%→100%). **제자리 회전("돌며")은 이동 아님 = no-move가 정답**(모델이 이 구분은 이미 함). 넛지는 **긍정 신호만, 배제 규칙 없이**(과적합 회피).
- **시퀀스 분류·튜닝은 few-shot으로** — 복합(plan)=뚜렷한 순차/동시 다른 동작("A한 뒤 B"). **경계(v2.15 갱신, #200 C안·#203 공격적 절충안)**: 명시적 입력은 보수적(명시 단일→단일, 명시 복합→구조 보존)이되, **추상·시적 영창명은 시퀀스 우선**("해석 갈리면 핵심 이미지 보존한 2~3단계"). 단일 유지 예외 = 정적 이미지(거울·구슬)·직접 원소주문(파이어볼)·수량1. 이전(v2.14)의 "애매하면 단일·시적/반복도 단일"에서 **뒤집힘** — 시적 다원소는 병렬로, 반복("세 번")은 기전(form×3)으로 편다. 트리거 = 프롬프트 7항 공격적 기본값 + `mockJudge.looksSequential`의 MOTION_SIGNALS(낙화·비행·검무 등 — 타임아웃 예산용)·PARALLEL_MARKERS(#201) **union**. 제약: heal/shield/buff 무단추가 금지·원소앵커 절대보존·seq≤3. **held-out 시퀀스율 ≥50%는 합격 아니라 과소분할 방지 바닥** — 진짜 합격은 per-case 기능품질(방향·앵커·무단추가0). 튜닝은 **트리거 단어 나열(게이트 과적합) 금지 → 다양한 few-shot + held-out 검증**. 미해소 허점 워치리스트 = #158.
