...
> **⚠️ 현행화 정정 (main 병합 시점, 2026-07-25 오후 이후) — 이 박스가 본문보다 우선한다.**
>
> 이 문서는 07-25 **오전 07:41** 스파이크 브랜치(`codex/r2-semantic-ir-spike`) 시점의 기록이다.
> 이후 아래 두 가지가 바뀌었으니 본문의 해당 서술은 **총괄 판단을 정본**으로 덮어읽어라:
>
> 1. **타임아웃**: 본문 §2·§7의 *"클라이언트 2,500ms 유지 / 3.2초로 늘리지 않는다"* 는
>    **뒤집혔다.** 총괄이 복합 전용 3.2초를 **승인**했고 #182로 main 병합됨
>    ([geminiJudge.ts](../src/spell/geminiJudge.ts) `SEQUENCE_TIMEOUT_MS = 3200`, 순차 마커 입력만 적용).
> 2. **버전 정합**: 본문은 production을 `v2.12`로 적었고 **라이브 probe로 사실 확인됨**
>    (move가 `custom-vector`·`angle` 반환). 그러나 **main `proxy/worker.js` 소스는 아직 v2.6-seq**
>    (destination 5종, directional 없음)라 **배포본이 소스보다 앞선 드리프트** 상태다.
>    main 클라이언트는 이미 custom-vector/angle을 소비한다(`sequencePlan.ts`, `spellPlanValidate.ts`).
>    → **main worker.js에서 재배포하면 production이 v2.12→v2.6로 조용히 퇴행한다.** 소스↔배포 정합이 남은 최우선 과제.
>
> 나머지 시행착오·폐기 이유·지연 원인 분석은 그대로 유효한 사료다.
