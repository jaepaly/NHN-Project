...
> ⚠️ **제출 전 갱신 필요**: 이후 프롬프트가 크게 진화했다 — 현재 배포는 **`meaning-v2.14`**로, ⑤ 영창 시퀀스(spell_plan 다단계·복합 3.2초 타임아웃), ⑥ 화면 절대 방향 이동(custom-vector), ⑦ 근접 참격 `slash` form, ⑧ 융합 이동 인식이 추가됐다. 아래 전문·데이터 흐름은 v2.5 스냅샷이므로, 제출 최종본에는 v2.14 기준으로 이 절과 프롬프트 전문을 갱신해야 한다(자세한 진화 이력은 `R2_PROGRESS.md`·`R2_SYSTEM_GUIDE.md`).
