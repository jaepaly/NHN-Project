...
따라서 "파고들어 벤다"의 이동감은 플레이어 좌표를 강제로 옮기는 대신 빠른 slash, beam, 잔상·도착 효과 같은 **form의 시각 언어**로 번역한다. 반면 소환수의 `behavior`는 소환된 개체 내부 움직임이므로 유지된다. 둘을 분리해 **플레이어 조작권은 코드가 지키고, 주문의 형태와 리듬은 AI가 설계**하게 했다.
