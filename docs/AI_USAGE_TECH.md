...
프롬프트는 LLM에게 **정해진 순서로 판단**하도록 지시한다. 아래는 **`meaning-v2.5` 시점 전문**이다 (`proxy/worker.js`).

> ⚠️ **제출 전 갱신 필요**: 이후 프롬프트가 크게 진화했다 — 현재 배포는 **`meaning-v2.15-abstract-seq`**로, ⑤ 영창 시퀀스(spell_plan 다단계·복합 3.2초 타임아웃), ⑥ 화면 절대 방향 이동(custom-vector), ⑦ 근접 참격 `slash` form, ⑧ 융합 이동 인식, **⑨ 추상·시적 영창명→2~3단계 시퀀스 확장(공격적 절충안 #209·#210 오버랩)**이 추가됐다. 아래 전문·데이터 흐름은 v2.5 스냅샷이므로, 제출 최종본에는 v2.15 기준으로 이 절과 프롬프트 전문을 갱신해야 한다(자세한 진화 이력은 `R2_PROGRESS.md`·`R2_SYSTEM_GUIDE.md`).
