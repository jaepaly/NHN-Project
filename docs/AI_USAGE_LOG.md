...
| 2026-07-14 | 이도원 | Codex | beam·wave 주문 폼 및 전투 마무리 | 직선 관통 광선과 전진형 파면의 다중 적 피해, 테스트 수치·긴 주문명 표시를 정리하고 R1 모듈을 `src/combat-core/`로 묶어 역할 경계를 명확화 | `feat/spell-renderer-forms`, 렌더러·전투 설정·주문 UI, `src/combat-core/` 구조 | `SpellSpec` 구조 유지, 폼 수치·적 HP는 임시값 |
