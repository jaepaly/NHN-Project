...
assert.equal(state.nodes.find((node) => node.id === 's1-boss')?.status, 'reachable');

graph.enter('s1-boss');
assert.equal(graph.isBossNode(graph.current().id), true);
assert.equal(graph.isFinalBossNode(graph.current().id), false);
assert.deepEqual(graph.choices().map((node) => node.kind), ['combat', 'altar']);
graph.enter('s2-combat');
assert.deepEqual(graph.progress(), { stage: 2, roomNumber: 1, totalVisitedRooms: 4 });
