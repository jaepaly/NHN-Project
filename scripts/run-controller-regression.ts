...
// 7) Phase 3.5 보상 풀 — 시드 랜덤 추첨·신규 패시브 (PROGRESSION_DESIGN §1)
{
  ...
  surge.addManaPickupRadiusMultiplier(RUN_REWARD_CONFIG.manaSurgePickupRadiusBonus);
  const restoredBySurge = surge.restoreMana(20);
  assert.equal(
    restoredBySurge,
    20 * (1 + RUN_REWARD_CONFIG.manaSurgeGainBonus),
  );
  assert.equal(
    surge.manaPickupRadiusMultiplier,
    1 + RUN_REWARD_CONFIG.manaSurgePickupRadiusBonus,
  );
  ...
  assert.equal(surge.manaPickupRadiusMultiplier, 1);
}
