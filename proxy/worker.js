...
7. 입력이 **시간 순서가 있는 복합 동작**("먼저 A 그다음 B", "A한 뒤 B")이거나 **동시 다원소 동작**("얼음과 불을 동시에")이면, spell(대표 하나)에 더해 spell_plan을 설계한다.
    ...
    "sequences": [
      ...
      { "durationWeight": 1, "behaviors": [
          { "type": "form", "powerWeight": 1, "tuning": { "damage": 2, "radius": 2 },
            "spec": { "name": "돌진 폭발", "effect": "damage", "target": "self", "element_primary": "fire", "element_secondary": null, "form": "nova", "size": "large", "speed": "normal", "status": ["burn"], "power": 0, "cost": 0 } } ] }
    ]
  }
}
...
spell_plan은 복합/순차·동시 동작일 때만 포함한다(그 외 생략). type은 form|move|wait, move는 element 필수·destination 5종(cast-point|target-direction|away-from-target|random-direction|arena-center)만.
