...
5. size와 speed를 정한다. 규모·빠르기 표현이 있으면 반드시 반영하고, 없으면 medium·normal로 둔다.
   - size: 조그만·작은·미세한=small / (표현 없음)=medium / 거대한·커다란·육중한=large / 하늘을 덮는·전장을 뒤덮는=huge
   - speed: 느릿느릿·천천히·서서히=slow / (표현 없음)=normal / 빠른·재빠른·순식간에·번개처럼=fast
6. effect가 summon인 주문에 한해, 소환수 움직임 묘사를 behavior(움직임 프로그램)로 설계한다.
