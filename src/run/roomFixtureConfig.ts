 ...
 * **방이 아니라 팝업**이었다. 도착(왼쪽)과 출구(오른쪽) 사이를 걸을 이유가 없어,
 * 포탈로 만든 좌→우 진행 구조가 이 방들에서만 무의미해졌다.
 *
 * 이제 중앙 설치물까지 걸어가 상호작용해야 보상이 열린다. 흐름:
 *   왼쪽 도착 → 중앙 이동 → 상호작용 → 3택 → 오른쪽 포탈
 *
...
export const ROOM_FIXTURE_CONFIG = {
  ...
  reachRadius: 44,
  /**
   * 등장 후 이 시간 동안은 무시. 설치물이 중앙이고 도착은 왼쪽 끝이라 겹칠 일이
   * 없지만, 시연 로드아웃처럼 스폰 지점이 바뀌는 경로가 있어 남긴다.
   */
  armDelayMs: 400,
...
export const ROOM_FIXTURE_GUIDE: Record<RoomFixtureKind, string> = {
  treasure: '방 중앙의 보물상자로 다가가라',
  altar: '방 중앙의 제단으로 다가가라 — 대가는 그때 정한다',
};
