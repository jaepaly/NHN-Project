 ...
 * 병렬(동시) 마커 — `~하면서`·`~하며`·`동시에`.
 *
 * 순차 마커와 **분리해서 둔다**. 타임아웃 분기는 "응답이 커질 입력인가"를 묻는 것이라
 * 순차·병렬을 다 세야 하지만, Mock의 폴백 plan은 절을 **순서대로** 실행하므로 병렬
 * 문장을 절로 쪼개면 "동시에"를 "차례로"로 바꿔버린다 — 의미가 뒤집힌다.
 */
// '며 '는 어간이 바뀌므로(달리며·쏘며·던지며) 접미사째로 잡는다. 뒤에 공백을 붙여
// '며칠'·'며느리' 같은 낱말을 피한다. 오탐은 값이 싸다 — 타임아웃 상한이 0.7초 늘 뿐이고
// 그 비용은 **응답이 실패할 때만** 발생한다. 반면 누락은 복합 영창을 통째로 폴백시킨다.
const PARALLEL_MARKERS = [
  '면서', '며 ', '동시에', ' while ', 'at the same time',
];

/**
 * 복합 영창으로 보이는가 — **판정 타임아웃 분기**(#180) 전용.
 *
 * 왜 순차만으로 부족한가(#200 조사에서 발견): 스키마는 `A하면서 B`(병렬)를 지원하고
 * 이슈도 그걸 "명시적 복합 영창"의 예로 들지만, 마커 목록에 순차만 있어서
 * "후퇴하면서 얼음창을 쏜다" 같은 병렬 문장이 전부 **2.5초 예산**에 걸려 있었다.
 * 복합 응답은 spell_plan을 실어 실측 1.86~2.55s라 그 경계에 그대로 부딪힌다
 * → 타임아웃 → Mock의 뭉툭한 단일 주문. 병렬도 같이 세어 복합 4.5초를 준다.
 */
/**
 * 이동·연속 동작 신호 — 명시적 순차 마커가 없어도 spell_plan(다단계)을 유발하는 입력 모양.
 * v2.14 move-fusion·추상 추출로 이런 입력이 plan(느림 ~2.2~2.3s)을 내는데, 마커가 없으면
 * `judgeTimeoutMs`가 2.5초만 줘서 절벽에 붙는다. 이 신호가 있으면 4.5초를 줘 폴백을 막는다.
...
const MOTION_SIGNALS = [
  ...
  // 후보 35~42: 복수·순환·전이 이동 문법
  '점멸', '전이', '가로질러', '가로지르', '되돌아', '한 바퀴', '돌아', '오가', '누비', '튕',
  // 후보 43: 시간차 반복 공격
  '연사', '속사', '연발', '잇달아',
  // 화면 방향 (custom-vector 이동 유발)
  '왼쪽', '오른쪽', '아래로', '앞으로', '뒤로',
  // 동작성 추상 이름
  '낙화', '비행', '활공', '추격', '순례', '선회', '행진', '왈츠', '무도', '답보', '검무',
  // 후보 58: 형성→변화→결말 plan을 유발하는 강한 상태 변화
  '갈라', '분열', '개화', '변신', '융합', '붕괴', '부활', '각성',
];
...
export function looksSequential(text: string): boolean {
  ...
  const padded = ` ${text} `;
  if (PARALLEL_MARKERS.some((marker) => padded.includes(marker))) return true; // 병렬 동시성(#201)
  return MOTION_SIGNALS.some((s) => text.includes(s)); // 이동·동작 신호(v2.15) — 둘은 합집합
}
...
const FORM_KEYWORDS: Record<SpellForm, string[]> = {
  ...
  // 근접 참격 (총괄 요청: 검·칼·베·참격·휘두 커버). 단 **'검' 단독은 '검은/검다'에 부분매칭**되어
  // 오탐(어둠 주문 잠식)이라 기존 관례('링'/'존' 제외)대로 넣지 않고 '대검/장검/검격'으로 안전하게 커버한다.
  slash: ['칼', '칼날', '대검', '장검', '검격', '참격', '참수', '도끼', '발톱',
          '베기', '벤다', '베어', '베는', '횡베기', '휘두르', '슬래시',
          'slash', 'sword', 'blade', 'cleave'],
};
