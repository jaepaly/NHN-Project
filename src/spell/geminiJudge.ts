 ...
 *   3) validateSpec 재검증 — 스키마 밖 값은 거부 (LLM을 신뢰하지 않음)
 *   4) 실패/타임아웃/무효 → MockJudge 폴백 (게임은 절대 멈추지 않는다)
 *
 * 프롬프트·API 키는 서버(worker.js)에 고정 — 클라이언트는 { text }만 보낸다.
 * 인터페이스 계약상 judge()는 throw하지 않고 항상 SpellSpec을 반환한다.
 */

const CACHE_PREFIX = 'incant:judge:';
...
export class GeminiJudge implements SpellJudge {
  ...
  private readCache(text: string): SpellSpec | null {
    try {
      const hit = localStorage.getItem(CACHE_PREFIX + text);
      if (!hit) return null;
      return validateSpec(JSON.parse(hit)); // 캐시도 재검증 (스키마 변경 대비)
    } catch {
      return null;
    }
  }

  private writeCache(text: string, spec: SpellSpec): void {
    try {
      localStorage.setItem(CACHE_PREFIX + text, JSON.stringify(spec));
    } catch {
