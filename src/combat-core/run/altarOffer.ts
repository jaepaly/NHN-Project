...
export const ALTAR_OFFER_CONFIG = {
   ...
   */
  /**
   * 파문 — **이번 제출에서는 제공하지 않는다** (총괄 결정 2026-08-07, #365).
   *
   * 설계: 에코와 같은 값(50)·같은 급이되 축이 다르다. 위력이 에코(0.7)보다 낮은 이유는
   * 대상이 늘어나 총 산출을 맞춰야 해서고, **적이 둘 이상일 때만 발동**한다 —
   * 보스전에서는 완전히 논다. 그 상황 의존성이 에코와의 균형이었다.
   *
   * 왜 뺐나: 최상위(`altar-high`)는 이미 4종이고 보상 카드 UI가 4장까지만 그린다
   * (`rewardCardOverlay`의 `slice(0, 4)`). 다섯 번째를 넣으면 하나가 조용히 잘리므로
   * "5종 중 4종 무작위" 같은 정책이 새로 필요한데, 콘텐츠 프리즈일에 감당할 변경이 아니다.
   * 최상위 선택지는 파문 없이도 충분하다는 판단.
   *
   * 설정·구현(`ProtoScene.scheduleSpellRipple`)은 **의도적으로 남긴다** — 되살릴 때
   * 다시 만들지 않아도 되게. 되살리려면 `HIGH_ALTAR_OFFERS`에 항목을 넣고,
   * `rewardCardOverlay`의 `KIND_LABELS`·`slice` 정책과 `rewardGlossary` 문장을 함께 복구한다.
   * 그 전에 R1이 짚은 조준 결함부터 고쳐야 한다: 현재 구현은 반경을 **피격 대상이 아니라
   * 플레이어 기준**으로 재고, 원본 대상도 실제 명중 대상이 아니라 예약 시점 최근접이다.
   */
  ripple: {
    delayMs: 260,
    powerScale: 0.55,
 ...
 * 거래 등급 — 3단(10 / 25 / 50).
 *
 * ⚠️ **최상위 안에 선택지가 여럿인 이유** (총괄 지적: "제단을 한 런에서 2회 이상
 * 방문하는 플레이어를 위해 다른 보상 하나 더"). 실측으로 한 런에 제단 2회가 3.2%이고,
 * 맵에 제단이 2개 이상인 경우가 21.7%다 — 일부러 노리면 더 잦다. 최상위 보상이
 * 하나뿐이면 두 번째 제단이 "이미 가진 걸 또 사거나 하위로 내려가는" 자리가 된다.
 *
 * 그래서 `altar-high` 한 등급 안에 **4종**(에코·성운 분열·원소 낙성·마력 궤적)을 두고
 * 이미 산 것은 `drawHighAltarOptions`가 걸러낸다. 두 번째 제단이 "남은 걸 줍는 자리"가
 * 아니라 **빌드 방향을 정하는 자리**가 되는 건 이 4종의 성격이 갈리기 때문이다.
 *
 * (종전 주석은 최상위 등급 자체가 둘이던 시절 것이었다 — `{50, echo}`·`{50, ripple}`.
 *  지금은 `{50, altar-high}` 하나이고 그 안에서 고른다. 파문은 #365로 제외됐다.)
 */
